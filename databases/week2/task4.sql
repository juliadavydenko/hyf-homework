...
CREATE TABLE smoothie (
	...
	,FOREIGN KEY (category) REFERENCES colorType(category)
...
CREATE TABLE colorType (
	...
	,type_id INT NOT NULL
	,category VARCHAR(255)
	,FOREIGN KEY (color_id) INT NOT NULL REFERENCES color(color_id)
	,FOREIGN KEY (type_id) INT NOT NULL REFERENCES type(type_id)
	,PRIMARY KEY (
		color_id
		,type_id
		)
